...
describe('Factory', function() {
  ...
  describe('safeTransferFrom', function() {
    it('should safeTransferFrom', async function() {
      ...
      const { logs } = await factoryContract.safeTransferFrom(
        hacker,
        holder,
        optionId0,
        2,
      ...
      expect(logs[0].event).to.be.equal('Issue')
      expect(logs[0].args._beneficiary).to.be.equal(holder)
      expect(logs[0].args._tokenId).to.be.eq.BN(totalSupply.toNumber() - 2)
      expect(logs[0].args._wearableIdKey).to.be.equal(hash)
      expect(logs[0].args._wearableId).to.be.equal(WEARABLES[optionId0].name)
      expect(logs[0].args._issuedId).to.eq.BN(issued - 1)
      ...
      expect(logs[1].args._wearableId).to.be.equal(WEARABLES[optionId0].name)
      expect(logs[1].args._issuedId).to.eq.BN(issued)

      expect(issued).to.be.eq.BN(2)
      balanceOfHolder = await erc721Contract.balanceOf(holder)
      expect(balanceOfHolder).to.be.eq.BN(2)
    })

    it('reverts when transferFrom by an allowed address', async function() {
      await assertRevert(
        factoryContract.safeTransferFrom(
          hacker,
          holder,
          optionId0,
          1,
